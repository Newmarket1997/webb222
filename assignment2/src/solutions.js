...
function firstCountry() {
  ...
  let search = firstUser.address.country
...
function formatNames() {
  ...
  for (let i = 0; i < Object.keys(users).length; i++) {
    console.log(users[i].name.first + ' ' + users[i].name.last)
  }
}
...
userUtils.getOldestUser = function () {
  ...
  users.forEach(function (user) {
    var ageParsed = Date.parse(user.birthDate)
    var age = getAgeInYears(ageParsed)
    if (age > largeAge) {
      oldestUser = user
      largeAge = age
    }
